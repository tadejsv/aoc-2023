...
struct Position {
    ...
    auto operator==(const Position& other) const -> bool {
        return row == other.row && col == other.col && steps == other.steps && dir == other.dir;
    }
};
...
auto
is_valid(const Position& pos, const Eigen::Index nrows, const Eigen::Index ncols) -> bool {
...
main()  // NOLINT
{
    ...
    for (Eigen::Index i{ 0 }; i < n_rows; ++i) {
        for (Eigen::Index j{ 0 }; j < n_cols; ++j) {
            board(i, j) = lines[static_cast<std::size_t>(i)][static_cast<std::size_t>(j)] - '0';
        }
    }
    ...
    while (!min_queue.empty()) {
        ...
        if (current.pos.col + 1 == board.cols() && current.pos.row + 1 == board.rows()) {
        ...
        switch (current.pos.dir) {
            case Up:
                ...
                break;
            case Down:
                next[0] = { current.pos.row + 1, current.pos.col, current.pos.steps + 1, Down };
                next[1] = { current.pos.row, current.pos.col - 1, 1, Left };
                next[2] = { current.pos.row, current.pos.col + 1, 1, Right };
                break;
            case Left:
                next[0] = { current.pos.row, current.pos.col - 1, current.pos.steps + 1, Left };
                next[1] = { current.pos.row + 1, current.pos.col, 1, Down };
                next[2] = { current.pos.row - 1, current.pos.col, 1, Up };
                break;
            case Right:
                next[0] = { current.pos.row, current.pos.col + 1, current.pos.steps + 1, Right };
                next[1] = { current.pos.row + 1, current.pos.col, 1, Down };
                next[2] = { current.pos.row - 1, current.pos.col, 1, Up };
                break;
        }
        ...
        for (const auto& next_pos : next) {
            if (is_valid(next_pos, nrows, ncols) && !considered.contains(next_pos)) {
                ...
                min_queue.emplace(next_pos, current.heat + board(next_pos.row, next_pos.col));
            }
        }
    }
