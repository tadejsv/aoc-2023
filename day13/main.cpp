...
auto
block_to_ints(const std::span<const std::string>& block, bool horizontal)
...
main()  // NOLINT
{
    ...
    for (const auto& block_inds : blocks) {
        ...
        const auto block_ints_h = block_to_ints(block_span, true);
        const auto block_ints_v = block_to_ints(block_span, false);
