    ...
    while (getline(file, line)) {
        ...
        for (size_t i = 0; i < line.length(); ++i) {
            ...
            if (bool(isdigit(chr))) {
                ...
                if (auto three_digit = find_digit(line, last_digit_pos, i, 3, three_letter_nums);
                    three_digit >= 0) {
                    digit = three_digit;
                }
                if (auto four_digit = find_digit(line, last_digit_pos, i, 4, four_letter_nums);
                    four_digit >= 0) {
                    digit = four_digit;
                }

                constexpr std::size_t len5{ 5 };
                if (auto five_digit = find_digit(line, last_digit_pos, i, len5, five_letter_nums);
                    five_digit >= 0) {
                    digit = five_digit;
                }
            }
