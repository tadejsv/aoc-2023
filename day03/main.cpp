...
auto
get_symbols(const std::vector<std::string>& lines) -> MatrixXi {
...
auto
get_digits(const std::vector<std::string>& lines) -> std::vector<Digit> {
    const Eigen::Index height{ static_cast<Eigen::Index>(lines.size()) };
    const Eigen::Index width{ static_cast<Eigen::Index>(lines.at(0).length()) };
...
main() {  // NOLINT
    ...
    for (const auto digit : digits) {
        ...
        Eigen::Index x_start = std::max(digit.index.at(1) - static_cast<Eigen::Index>(digit.length), 0L);
        ...
        auto sub_block = symbols.block(y_start, x_start, y_end - y_start + 1L, x_end - x_start + 1L);
