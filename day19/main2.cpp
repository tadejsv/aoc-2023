...
class Interval {
  public:
    ...
    [[nodiscard]] auto empty() const -> bool { return lower >= upper; }
    [[nodiscard]] auto size() const -> long long { return upper - lower; }
};
...
class Item {
  public:
    ...
    [[nodiscard]] auto size() const -> long long {
        return intervals[x].size() * intervals[m].size() * intervals[a].size() *
               intervals[s].size();
    }
};
