...
auto compare_ranges(const RangeIndex &range_1, const RangeIndex &range_2) -> bool {
...
class RangeMap {
  ...
  public:
    ...
    auto sort() -> void { std::sort(ranges.begin(), ranges.end(), compare_ranges); }
        ...
        for (const auto &key_range : key_ranges) {
            ...
            for (const auto &range : ranges) {
                ...
                if (range.key_start > current_pos) {
                    val_ranges.push_back(
                        std::array<long long, 2>{current_pos, range.key_start - 1LL});
                ...
                const auto val_start =
                    current_pos + (range.val_start - range.key_start);
...
int main() { // NOLINT
        ...
        if (line.contains("seeds:")) {
            ...
            for (size_t i{0}; i < seeds_str.size(); i += 2) {
                ...
                seed_ranges.push_back(
                    std::array<long long, 2>{start, start + length - 1LL});
            }
        } else if (line.size() < 3) {
            continue;
    ...
    for (auto const &range : final_ranges) {
