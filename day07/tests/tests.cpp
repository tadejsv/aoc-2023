...
TEST_CASE("Test hand constructor") {
    auto test_case = GENERATE(
        std::make_pair("AAAAA", 6),
        std::make_pair("TAAAA", 5),
        std::make_pair("TAAAT", 4),
        std::make_pair("TAAA2", 3),
        std::make_pair("TA1AT", 2),
        std::make_pair("TA17T", 1),
...
TEST_CASE("Test hand comparison") {
    std::tuple<const char*, const char*, bool> test_case = GENERATE(
        std::make_tuple("AAAAA", "22345", false),
    ...
    REQUIRE(
        compare_hands(Hand(std::get<0>(test_case)), Hand(std::get<1>(test_case)))
        == std::get<2>(test_case)
    );
}
