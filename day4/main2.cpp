...
auto
intersection(const std::unordered_set<int>& set1, const std::unordered_set<int>& set2)
...
struct Card {
    ...
    Card(std::string_view line) {
        ...
        const auto our_numbers_str = utils::split_string(winning_our_numbers.at(0), ' ');
        const auto winning_numbers_str = utils::split_string(winning_our_numbers.at(1), ' ');
...
main() {  // NOLINT
    ...
    for (size_t i{ 0 }; i < lines.size(); i++) {
        ...
        const auto card_wins = card.wins();

        for (size_t j = i + 1; j < std::min(i + 1 + static_cast<size_t>(card_wins), lines.size());
             j++) {
            cards[j] += cards[i];
        }
    }
