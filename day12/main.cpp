...
auto operator<<(std::ostream &outs, const State &state) -> std::ostream & {
    outs << "State: {"
...
auto check_damaged(std::string_view row,
    ...
    // If this is the last character in row - did all the groups get filled?
    if (row.size() - 1 == state.current_ind) {
        if (state.group_found == groups[state.group_ind] - 1 &&
            state.group_ind == groups.size() - 1) {
            return 1;
        }
        return 0;
    }
...
auto evaluate(std::string_view row,
    const std::vector<int> &groups,
    const State &state,
    std::queue<State> &queue) -> int {
    ...
    // Otherwise we have '?'
    return check_empty(row, groups, state, queue) +
           check_damaged(row, groups, state, queue);
};
